...
from ..evaluation.neural_eval import evaluate, evaluate_cuda
...
def train_one_epoch_cuda(model: BaseModule, 
                    batchs: list[Batch],
                    criterion: torch.nn.CrossEntropyLoss,
                    optimizer: torch.optim.Optimizer):
    ...
    for btach in tqdm(batchs):
        ...
        logit = model(btach.input_ids.to("cuda"), btach.attention_masks.to("cuda"))
        loss = criterion(logit, btach.labels.to("cuda"))
...
def train_model_cuda(model: BaseModule, total_epochs:int, train_samples: list[Sample], 
                training_batch_size: int, pad_id: int, 
                test_batches: list[Batch], criterion: torch.nn.CrossEntropyLoss,
                early_stop: bool = False, lr: float = 1e-4, random_state: int = 43):
    history = []
    ...
    for epoch in tqdm(range(total_epochs)):
        _epoch_train_batch = create_batch_partial(in_samples=train_samples)
        train_loss, train_preds, train_labels = train_one_epoch_cuda(model=model, batchs=_epoch_train_batch, 
                                                                criterion=criterion, optimizer=optimizer)
        test_loss, test_preds, test_labels = evaluate_cuda(model, test_batches, criterion)
